...
namespace CareFusion.ITSystemSimulator
{
    ...
    public partial class FMain : Form
    {
        ...
        private void txtArticleFilter_TextChanged(object sender, EventArgs e)
        {
            ...
            _stockModel.Filter = txtArticleFilter.Text;
        ...
        private void btnReloadStock_Click(object sender, EventArgs e)
        {
            ...
            _stockModel.Clear();
            bgStock.RunWorkerAsync(txtArticleFilter.Text);
        }
        ...
        private void StorageSystem_StockUpdated(IStorageSystem sender, IArticle[] articleList)
        {
            ...
            _stockModel.Update(articleList);
        ...
        /// <summary>
        /// Handles the Click event of Loading the Excel sheet in Grid
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnImportFromSelect_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                openFileDialog.Title = "Load the Excel File";
                openFileDialog.FileName = "";
                openFileDialog.Filter = "Excel File|*.xlsx;*.xls;*.csv";

                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    _xlWorkbook = _xlApp.Workbooks.Open(openFileDialog.FileName);
                    _xlWorksheet = _xlApp.ActiveSheet;
                    _range = _xlWorksheet.UsedRange;

                    // ADD ROWS TO THE GRID.
                    for (int iRow = 2; iRow <= _xlWorksheet.Rows.Count; iRow++)  // START FROM THE SECOND ROW. (FIRST ROWS ARE HEADERS)
                    {
                        if (_xlWorksheet.Cells[iRow, 1].value == null) 
                        {
                            break;      // BREAK LOOP.
                        }
                        else
                         {
                            // CREATE A OBJECT ARRAY USING THE VALUES IN EACH ROW OF THE SHEET.
                            Object [] row = new Object[]
                            {
                                _xlWorksheet.Cells[iRow, 1].Value,
                                _xlWorksheet.Cells[iRow, 2].Value,
                                _xlWorksheet.Cells[iRow, 3].Value,
                                _xlWorksheet.Cells[iRow, 4].Value,
                                _xlWorksheet.Cells[iRow, 5].Value,
                                _xlWorksheet.Cells[iRow, 6].value = _xlWorksheet.Cells[iRow, 6].value.ToString() == "Y" ? true : false,
                                _xlWorksheet.Cells[iRow, 7].Value,
                                _xlWorksheet.Cells[iRow, 8].Value,
                                _xlWorksheet.Cells[iRow, 9].Value,
                                _xlWorksheet.Cells[iRow, 10].Value,
                                _xlWorksheet.Cells[iRow, 11].Value,
                                _xlWorksheet.Cells[iRow, 12].Value,
                                _xlWorksheet.Cells[iRow, 13].Value
                            };

                            _stockDeliveryModel.StockDeliveryItems.Rows.Add(row);
                        }
                    }
                    dataGridDeliveryItems.DataSource = _stockDeliveryModel.StockDeliveryItems;
                    btnSendStockDeliveries.Enabled = true;
            ...
            {
                // Log the Error for Analysing or Production Issues
            ...
            finally 
            {
                ...
                //System.Runtime.InteropServices.Marshal.ReleaseComObject(_xlWorksheet);
            }
